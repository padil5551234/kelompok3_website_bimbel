...
/**
 * Test Admin Interface and Materials Display (FIXED)
 * This script verifies that the integrated course system is working properly
 * Fixed to use Spatie's role system properly
 */
...
echo "🧪 Testing Admin Interface and Materials Display (FIXED)...\n\n";
...
try {
    ...
    // Test 4: Check admin users (FIXED - using Spatie's role system)
    echo "4️⃣ Testing Admin Users (Fixed):\n";
    try {
        $adminUsers = App\Models\User::role('admin')->get(); // Using Spatie's role scope
        ...
        if ($adminUsers->count() > 0) {
            ...
            echo "   ⚠️  No admin users found with 'admin' role\n";
            echo "   💡 You may need to assign the admin role to a user\n";
        }
    } catch (Exception $e) {
        echo "   ❌ Error checking admin users: " . $e->getMessage() . "\n";
        echo "   💡 This might be normal if roles haven't been set up yet\n";
    }
    echo "\n";
    
    // Test 5: Check tutor users (FIXED - using Spatie's role system)
    echo "5️⃣ Testing Tutor Users:\n";
    try {
        $tutorUsers = App\Models\User::role('tutor')->get(); // Using Spatie's role scope
        echo "   Total Tutor Users: " . $tutorUsers->count() . "\n";
        
        if ($tutorUsers->count() > 0) {
            foreach ($tutorUsers as $tutor) {
                echo "   ✅ Tutor: {$tutor->name} ({$tutor->email})\n";
            }
        } else {
            echo "   ⚠️  No tutor users found\n";
        }
    } catch (Exception $e) {
        echo "   ❌ Error checking tutor users: " . $e->getMessage() . "\n";
    }
    echo "\n";
    
    // Test 6: Check routes are working
    echo "6️⃣ Testing Route Availability:\n";
    $routes = [
    ...
    // Test 7: Check if our created course is accessible
    echo "7️⃣ Testing Our Created Course:\n";
    $ourCourse = App\Models\PaketUjian::where('nama', 'like', '%Matematika Dasar%')->first();
    if ($ourCourse) {
        echo "   ✅ Course found: {$ourCourse->nama}\n";
        echo "   Course ID: {$ourCourse->id}\n";
        echo "   Edit URL: http://127.0.0.1:8000/admin/integrated/course/{$ourCourse->id}/edit\n";
        
        $courseMaterials = App\Models\Material::where('batch_id', $ourCourse->id)->get();
        echo "   Total Materials: " . $courseMaterials->count() . "\n";
        
        $chapter1Count = $courseMaterials->where('chapter_number', 1)->count();
        echo "   Chapter 1 Materials: " . $chapter1Count . "\n";
    } else {
    ...
    echo "✅ Fixed role system compatibility\n\n";
    ...
    echo "🔧 ROLE ASSIGNMENT (If needed):\n";
    echo "===============================\n";
    echo "If you get role errors, you may need to assign roles manually:\n";
    echo "php artisan tinker\n";
    echo "\\App\\Models\\User::where('email', 'admin@course.com')->first()->assignRole('admin');\n\n";
