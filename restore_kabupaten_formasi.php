...
/**
 * Script PHP untuk mengisi data kabupaten/kota berdasarkan formasi BPS
 * Menghubungkan dengan database dan menggunakan Eloquent ORM Laravel
 * 
 * Usage: php restore_kabupaten_formasi.php
 * 
 * Pastikan Laravel sudah terinstall dan autoload vendor tersedia
 */
...
$databaseConfig = [
    ...
    'database' => $_ENV['DB_DATABASE'] ?? 'tryout',
...
try {
    ...
    $pdo = new PDO(
        "mysql:host={$databaseConfig['host']};port={$databaseConfig['port']};dbname={$databaseConfig['database']};charset=utf8mb4",
    ...
    // Cek apakah tabel kabupaten ada
    $stmt = $pdo->query("SHOW TABLES LIKE 'kabupaten'");
    if ($stmt->rowCount() === 0) {
        echo "❌ Tabel 'kabupaten' tidak ditemukan. Membuat tabel baru...\n";
        
        // Buat tabel kabupaten jika belum ada
        $createTableSQL = "
        ...
        echo "✅ Tabel 'kabupaten' berhasil dibuat\n";
    }
    
    // Kosongkan data kabupaten existing
    echo "🗑️ Menghapus data kabupaten existing...\n";
    $pdo->exec("TRUNCATE TABLE kabupaten");
    
    // Data kabupaten berdasarkan formasi BPS
    $kabupatenData = [
        // =====================================================
        ...
        // =====================================================
        ['1801', '09', 'Kabupaten Lampung Barat'],
    ...
    echo "📝 Memulai pengisian data kabupaten...\n";
    
    // Prepare statement untuk insert
    $stmt = $pdo->prepare("
        INSERT INTO kabupaten (kode, kode_provinsi, nama, created_at, updated_at) 
        VALUES (?, ?, ?, NOW(), NOW())
    ");
    ...
    try {
        foreach ($kabupatenData as $kabupaten) {
            [$kode, $kodeProvinsi, $nama] = $kabupaten;
            
            $stmt->execute([$kode, $kodeProvinsi, $nama]);
            $successCount++;
            
            if ($successCount % 50 == 0) {
                echo "✅ Processed {$successCount} kabupaten...\n";
            }
        }
        ...
        $pdo->rollback();
        echo "❌ Error dalam transaksi: " . $e->getMessage() . "\n";
        $errorCount++;
    }
    ...
    // Cek total kabupaten
    $stmt = $pdo->query("SELECT COUNT(*) as total FROM kabupaten");
    $total = $stmt->fetch()['total'];
    echo "Total kabupaten yang diinput: {$total}\n";
    
    // Cek jumlah kabupaten per provinsi
    $stmt = $pdo->query("
        SELECT 
            f.nama as provinsi,
    ...
    echo "\nJumlah kabupaten per provinsi:\n";
    while ($row = $stmt->fetch()) {
        echo "- {$row['provinsi']}: {$row['jumlah_kabupaten']} kabupaten/kota\n";
    }
