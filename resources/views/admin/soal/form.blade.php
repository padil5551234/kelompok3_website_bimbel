                        ...
                        <div id="form-content">
                                    ...
                                    <div class="form-group row required">
                                        ...
                                        <div class="col-lg-2 col-md-3">
                                            <select class="form-control @error('jenis_soal') is-invalid @enderror" name="jenis_soal" id="jenis_soal">
                                                ...
                                                <option value="twk" @if (old('jenis_soal', $soal->jenis_soal) == 'twk') selected @endif>TWK</option>
                            ...
                            <div class="nontkp">
                                <div class="form-group row required">
                                    ...
                                    <div class="col-lg-3 col-md-3">
                                        <select class="form-control @error('kunci_jawaban') is-invalid @enderror" name="kunci_jawaban" id="kunci_jawaban">
                                            ...
                                            @else
                                                @for($i = 0; $i < 5; $i++)
                                                <option value="{{ $i }}" @if (old('kunci_jawaban', $soal->kunci_jawaban) == $i) selected @endif>{{ chr($i + 65) }}</option>
                                                @endfor
                                            @endif
                                        </select>
                                        @error('kunci_jawaban')
                                ...
                                <div class="form-group row required">
                                    ...
                                    <div class="col-lg-3 col-md-3">
                                        <input type="number" class="form-control @error('nilai_benar') is-invalid @enderror" placeholder="Nilai Benar" name="nilai_benar" value="{{ old('nilai_benar', $soal->poin_benar ?? 5) }}" required>
                                    </div>
                                    @error('nilai_benar')
                                        <div class="invalid-feedback">
                                            <h6>{{ $message }}</h6>
                                    ...
                                    <div class="col-lg-3 col-md-3">
                                        <input type="number" class="form-control @error('nilai_kosong') is-invalid @enderror" placeholder="Nilai Kosong" name="nilai_kosong" value="{{ old('nilai_kosong', $soal->poin_kosong ?? 0) }}" required>
                                    </div>
                                    @error('nilai_kosong')
                                        <div class="invalid-feedback">
                                            <h6>{{ $message }}</h6>
                                    ...
                                    <div class="col-lg-3 col-md-3">
                                        <input type="number" class="form-control @error('nilai_salah') is-invalid @enderror" placeholder="Nilai Salah" name="nilai_salah" value="{{ old('nilai_salah', $soal->poin_salah ?? 0) }}" required>
                                    </div>
                                    @error('nilai_salah')
                                        <div class="invalid-feedback">
                                            <h6>{{ $message }}</h6>
...
<script type="text/javascript">
    ...
    $(function() {
        function uploadImage(file, $editor) {
            ...
            $.ajax({
                url: '{{ route('admin.soal.upload-image') }}',
                method: 'POST',
                data: data,
                processData: false,
                contentType: false,
                ...
                success: function (resp) {
                    if (resp && resp.url) {
                        ...
                        toastr.error('Format respons upload tidak valid.');
                    }
                },
                error: function () {
                    toastr.error('Upload gambar gagal. Coba lagi.');
                }
            });
        }
        ...
        initSummernote('#soal', 250);
        initSummernote('#pembahasan', 150);
        initSummernote('.jawaban', 100);
