...
<script>
    ...
    $(function() {
        ...
        $('#modal-form form').on('submit', function(e) {
            ...
            .fail((errors) => {
                ...
                if (errors.status === 422 && errors.responseJSON) {
                    // Validation errors
                    const validationErrors = errors.responseJSON.errors || {};
                    const errorList = [];
                    
                    Object.keys(validationErrors).forEach(function(key) {
                        errorList.push(validationErrors[key][0]);
                    });
    ...
    function addForm(url) {
        ...
        // Load and populate package options
        loadPackageOptions();
    }
    ...
    function editData(url) {
        ...
        $.get(url)
            .done((response) => {
                ...
                // Load packages and select the ones associated with this ujian
                loadPackageOptions(response.id);
            })
            .fail((errors) => {
                alert('Tidak dapat menampilkan data.');
                return;
    ...
    function loadPackageOptions(ujianId = null) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                packages.forEach(function(paket) {
                    const option = new Option(paket.nama, paket.id, false, false);
                    $('#paket_ujian').append(option);
                });
                
                // If editing, load the selected packages
                if (ujianId) {
                    $.ajax({
                        url: `/admin/ujian/${ujianId}/packages`,
                        type: 'GET',
                        success: function(selectedPackages) {
                            $('#paket_ujian').val(selectedPackages).trigger('change');
                        },
                        error: function() {
                            console.log('Could not load selected packages');
                        }
                    });
                }
                
                $('#paket_ujian').trigger('change');
            },
            error: function() {
                toastr.error('Tidak dapat memuat data paket ujian');
            }
        });
    }
